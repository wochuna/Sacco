...
mapping which relates a ``User`` to a ``Savingwalletunt``::
    ...
    class User(Base):
        ...
        accounts: Mapped[List[Savingwalletunt]] = relationship(
        ...
        def _balance_setter(self, value: Optional[Decimal]) -> None:
            ...
            if not self.accounts:
                account = Savingwalletunt(owner=self)
            else:
        ...
        def _balance_expression(cls) -> SQLColumnExpression[Optional[Decimal]]:
            return cast(
                ...
                Savingwalletunt.balance,
            )

The above hybrid property ``balance`` works with the first
``Savingwalletunt`` entry in the list of accounts for this user.   The
...
``Savingwalletunt`` will be present:
...
we can adjust our ``Savingwalletunt`` example to aggregate the balances for
*all* accounts, and use a correlated subquery for the column expression::
    ...
    class User(Base):
        ...
        def _balance_expression(cls) -> SQLColumnExpression[Decimal]:
            return (
                select(func.sum(Savingwalletunt.balance))
